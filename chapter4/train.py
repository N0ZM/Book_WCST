...
plt.savefig(os.path.join(
    os.path.dirname(__file__), 'result1.png'), dpi=300)
plt.show()
...
for epoch in range(n_epoch):
    ...
    state = reasoner(q)
    loss_data = policy.fit(state, q)
    loss_list.append(loss_data.sum())
    print(f'epoch: {epoch}, total loss: {loss_data.sum()}')
...
plt.ylabel('Loss')
plt.savefig(os.path.join(
    os.path.dirname(__file__), 'result2.png'), dpi=300)
plt.show()
