...
def exec_one_trial(policy, reasoner, correct_attr, 
                   prev_result_policy, prev_result_reasoner):
    ...
    if result_reasoner == 'Incorrect' and \
        prev_result_reasoner == 'Incorrect' \
            and is_anomaly_reasoner:
        reasoner.init()
    if result_policy == 'Incorrect' and \
        prev_result_policy == 'Incorrect' and is_anomaly_policy:
        policy.init()
    ...
    return result_policy, result_reasoner
...
def exec_WCST(n_test, n_trial, file_name):
    ...
    for i in range(n_test):
        ...
        for j in range(n_trial):
            ...
            result_policy, result_reasoner = exec_one_trial(
                policy, reasoner, correct_attr, 
                prev_result_policy, prev_result_reasoner)
            if result_policy == 'Correct':
                result_test += 1
                prev_result_policy = 'Correct'
            else:
                prev_result_policy = 'Incorrect'
            if result_reasoner == 'Correct':
                prev_result_reasoner = 'Correct'
            else:
                prev_result_reasoner = 'Incorrect'
            print(f'j: {j}, {result_policy}')
...
exec_WCST(50, 100, os.path.join(
    result_path, 'policy_with_reasoner'))
